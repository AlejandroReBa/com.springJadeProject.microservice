...
public class AgentsManager {
    ...
    public void startup(){
        if (mainContainer == null){
            ...
            try {
                ...
                throw new JadeAgentException("ERROR: ControllerException - main container failed when it was being started -> "
                + ex.getMessage());
            }
        }
    }
    ...
    public void shutdown(){
        if (mainContainer != null){
            try {
                ...
                throw new JadeAgentException("ERROR: StaleProxyException - main container failed when it was being finished -> "
                + ex.getMessage());
            }
        }
    }
    ...
    public static void addAgentToMainContainer(String nickname, Agent agent){
        try {
            ...
            throw new JadeAgentException("ERROR: StaleProxyException - exception trying to init again the agent " + nickname + " ! -> "
                    + ex.getMessage());
        }
    }
    ...
    public static void takeDownAgent(String nickname, Agent agent) {
        try {
            if (agent == null) {
                ...
                throw new JadeAgentException("ERROR: The agent " + nickname + " that you are trying to shut down is" +
                        " not initiated or has been already shutted down");
            }
        } catch (ControllerException ex) {
            throw new JadeAgentException("ERROR: ControllerException - exception shutting down the agent " + nickname + " ! -> "
                    + ex.getMessage());
        }
    }
